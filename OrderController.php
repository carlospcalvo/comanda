...
class OrderController{
    ...
    public function deleteOne(Request $request, Response $response, $args){    
        $table = Table::where('id', $request->getParsedBody()['id_table'])->get(); 
	$table[0]['status'] = 'Cerrada';
        if(Order::destroy($args) && $table[0]->save()){
        	$response->getBody()->write(json_encode(['Exito'=>"Pedido eliminado con éxito!"],JSON_PRETTY_PRINT));
	} else {
        	$response->getBody()->write(json_encode(['Error'=>"No se pudo eliminar el pedido."],JSON_PRETTY_PRINT));
	}
        return $response;
    }
